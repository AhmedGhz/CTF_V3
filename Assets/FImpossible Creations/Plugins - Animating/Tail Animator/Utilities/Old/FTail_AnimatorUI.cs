        ...
        protected Quaternion CalculateFor2D(FTail_Point tailPoint)
        {
            ...
            if (FullCorrection)
            {
                ...
                if (LookUpMethod != FELookUpMethod.Parental)
                {
                    ...
                    {
                        targetRotation = TopDownAnglePosition2D(lookingAt, startLookPos);
                    }
            ...
            else
            {
                ...
                if (startLookPos - lookingAt != Vector3.zero) targetRotation = TopDownAnglePosition2D( startLookPos, lookingAt);
