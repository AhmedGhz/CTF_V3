...
namespace MalbersAnimations
{
    ...
    public class TransformAnimation : ScriptableObject
    {
        ...
        public void PlayTransform(MonoBehaviour coroutine)
        {
            coroutine.StartCoroutine(PlayTransformAnimation(coroutine.transform));
        }
        ...
        protected IEnumerator PlayTransformAnimation(Transform item)
        {
            if (item != null)
            {
                ...
                var TargetRot = StartRot * Quaternion.Euler(Rotation);
                ...
                while ((time > 0) && (elapsedTime <= time) && item != null)
                {
                    ...
                    if (UsePosition)
                    {
                        item.localPosition = Vector3.LerpUnclamped(StartPos, TargetPos, resultPos);
                    }

                    if (UseRotation)
                    {
                        item.transform.localRotation = Quaternion.LerpUnclamped(StartRot, TargetRot, resultRot);
                    }

                    if (UseScale)
                    {
                        item.transform.localScale = Vector3.LerpUnclamped(StartScale, TargetScale, resultSca);
                    }
                ...
                if (UseRotation)
                {
                    ...
                    item.transform.localRotation = Quaternion.LerpUnclamped(StartRot, TargetRot, FresultRot);
                }
                if (UseScale)
                {
