...
namespace MTAssets.UltimateLODSystem
{
    ...
    */
    ...
    public class UltimateLevelOfDetail : MonoBehaviour
    {
        ...
        private int forcedToShowOnlyALodIndependentOfDistance = -1; //<-- -1 = Will not force to show a LOD
        ...
        public void OnRenderObject()
        {
            ...
            //If this ulod component is forced to show only a defined LOD, independent of camera distance
        ...
        //Exclusive methods to make this component work with HDRP 
        //This is necessary because HDRP cannot call the OnRenderObject Hook, only URP and Built-In can do this
        //these methods will simulate OnRenderObject calling, as the URP and Built-In RP does.

        private IEnumerator OnRenderObject_HookEmulationForHDRP()
        {
            //Create a WaitForEndOfFrame variable for avoid Garbage
            WaitForEndOfFrame waitForEndOfFrame = new WaitForEndOfFrame();

            //Start a loop that is breaked when this component is disabled
            while (this.enabled == true)
            {
                //Wait for end of frame
                yield return waitForEndOfFrame;

                //Call the OnRenderObject hook
                OnRenderObject();
            }
        }
        ...
        {
            //If is not playing or not have a scan, cancel the execution
            if (Application.isPlaying == false || currentScannedMeshesList.Count == 0)
                return;

            //Check if this project is using HDRP as Render Pipeline
            bool thisProjectIsUsingHdrp = false;
            if (CurrentRenderPipeline.haveAnotherSrpPackages == true && String.Equals(CurrentRenderPipeline.packageDetected, "HDRP") == true)
                thisProjectIsUsingHdrp = true;
            //If this not is the HDRP renderization, cancel
            if (thisProjectIsUsingHdrp == false)
                return;

            //Start a coroutine to simulate call on OnRenderObject method
        ...
        {
            //This method will force this component to show only a level of detail, independent of distance of camera
            if (force == true)
            {
                //If provided a level smaller than 0
                if (level < 0)
                {
                    Debug.LogError("It was not possible to force a LOD on the ULOD component of \"" + this.gameObject.name + "\". The level provided is less than zero.");
                    return;
                }
                if (level > levelsOfDetailToGenerate)
                {
                    Debug.LogError("It was not possible to force a LOD on the ULOD component of \"" + this.gameObject.name + "\". The level provided is greater than the number of levels of detail that this component has generated.");
                    return;
                }

                //Set the new level on variable
                forcedToShowOnlyALodIndependentOfDistance = level;
            }
            //If is not desired to force more, disable
            if (force == false)
        ...
        {
            //This method will return true if this component is forced to show only a level of detail, independet of distance of camera
            return ((forcedToShowOnlyALodIndependentOfDistance == -1) ? false : true);
        }
