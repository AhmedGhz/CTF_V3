...
namespace MTAssets.UltimateLODSystem
{
    ...
    */
    ...
    public class UltimateLevelOfDetailOptimizer : MonoBehaviour
    {
        ...
        {
            //This method will return true if a determined ULOD is present in ulods to be ignored list
            bool isPresent = false;

            //Run the loop checker
            foreach (UltimateLevelOfDetail thisUlod in ulodsToBeIgnored)
                if (thisUlod == ulod)
                {
                    isPresent = true;
                    break;
        ...
        private IEnumerator UlodOptimizationLoop()
        {
            ...
            while (true == true)
            {
                ...
                if (enableOptimizationTasks == true)
                {
                    ...
                    //Run a loop to check if each ulod in scene will be ignored or not
                    for (int i = 0; i < instructionsToMakeOnUlods.Length; i++)
                    {
                        bool ulodsToBeIgnoredContainsThisUlod = isThisUlodPresentOnUlodsToBeIgnored(runtimeInstancesDetector.instancesOfUlodInThisScene[i]);
                        if (ulodsToBeIgnoredContainsThisUlod == true)
                            instructionsToMakeOnUlods[i] = 2;
                        if (ulodsToBeIgnoredContainsThisUlod == false)
                ...
                if (enableOptimizationTasks == false)
                    ...
                    {
                        bool ulodsToBeIgnoredContainsThisUlod = isThisUlodPresentOnUlodsToBeIgnored(ulod);
                        if (ulodsToBeIgnoredContainsThisUlod == false)
