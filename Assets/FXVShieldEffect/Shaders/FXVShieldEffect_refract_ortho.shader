...
Shader "FXV/FXVShieldEffectRefract_ortho" {
	Properties {
		_Color ("Color", Color) = (1,1,1,1)
		_ColorRimMin("ColorRimMin", Range(-10,10)) = 0.6
		_ColorRimMax("ColorRimMax", Range(-10,10)) = 1.0
...
//#endif
			...
			{
				float vdn = 1.0 - max(dot(half3(0.0, 0.0, 1.0), i.rimN), 0.0);

				float depthVisibility = 1.0;

				float depthValue = SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, UNITY_PROJ_COORD(i.screenPos));
				float depthDiff = (depthValue - i.screenPos.z / _ProjectionParams.z);
				depthVisibility = max(_Preview, step(-depthDiff, 0.0));
#if USE_DEPTH_OVERLAP_RIM
				float depthRim = (1.0 - _Preview)*(1.0 - min(1.0, (depthDiff * _ProjectionParams.z) / _OverlapRim));
#endif
