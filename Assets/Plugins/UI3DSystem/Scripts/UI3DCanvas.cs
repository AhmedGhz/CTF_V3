...
public class UI3DCanvas : MonoBehaviour
{
    ...
    private List<UI3DDepthObject> depthObjects = new List<UI3DDepthObject>();
    ...
    {
        Profiler.BeginSample("Reinitialize UI 3D Canvas");
        canvas = GetComponent<Canvas>();
        canvasRect = canvas.GetComponent<RectTransform>();
    ...
    public void UnregisterUIParticleDepthObject(UI3DDepthObject obj)
    {
        depthObjects.Remove(obj);
        SetDirty();
    }

    public void RegisterUIParticleDepthObject(UI3DDepthObject obj)
    {
    ...
    {
        Profiler.BeginSample("Refresh UI 3D Canvas Mask");
        if (mask == null)
            Reinitialize();
    ...
    private int SortDepthObjects(UI3DDepthObject x, UI3DDepthObject y)
    {
    ...
    private void OnCanvasCameraPreRender()
    {
		if(maskCamera == null)
			Reinitialize();
        Shader.SetGlobalFloat("_UI3DCanvasZMin", minDepth);
        Shader.SetGlobalFloat("_UI3DCanvasZMax", maxDepth);
        Shader.SetGlobalTexture("_UIDepthTex", maskCamera.targetTexture);
        Shader.SetGlobalVector("_UI3DDepthTexPosParams", DepthTexPosParams);
