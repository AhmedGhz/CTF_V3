...
public class UI3DShaderEditor : ShaderGUI
{
    ...
    protected static void DrawClippingMaskSettings(Material material)
    {
        ...
        if(useClippingMask != material.IsKeywordEnabled("USE_CLIPPING_MASK"))
        {
			if(useClippingMask)
			{
				...
				material.EnableKeyword("USE_CLIPPING_MASK_ON");//ASE support
			}
			else
			{
				material.DisableKeyword("USE_CLIPPING_MASK");
				material.DisableKeyword("USE_CLIPPING_MASK_ON");//ASE support
			}
        }
    ...
    protected void DrawBlendingSettings(Material material)
    {
